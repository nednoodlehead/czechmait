    ...
    for operator_pair in operators:
        ...
        while True:
            ...
            if new_tile is None:
                ...
                piece_instances.append(Move(tile, new_tile, piece(color), None))
                break
        ...
        if not new_tile:
            ...
            if board.is_occupied(new_tile):
                if board.is_occupied_enemy(new_tile, color):
                    possible_moves.append(Move(tile, new_tile, Knight(color), None))
            else:
                possible_moves.append(Move(tile, new_tile, Knight(color), None))
...
def king_analysis(board, tile):
    ...
    for coords in spots:
        ...
        if not board.is_occupied(new_tile):
            possible_moves.append(Move(tile, new_tile, King(color), None))
        elif board.is_occupied_enemy(new_tile, color):
            possible_moves.append(Move(tile, new_tile, King(color), None))
    return possible_moves
        ...
        if board.is_occupied(front_tile) is False:
            if front_tile[1] == color.pawn_promotion_rank:
                for promotion_option in promotion_list:
                    ...
                    ret_list.append(Move(tile, front_tile, promotion_option(color), None))
            else:
                ret_list.append(Move(tile, front_tile, Pawn(color), None))
            ...
            # it is allowed to double jump
                ...
                ret_list.append(Move(tile, two_infront, Pawn(color), DoublePawnMove(front_tile, color) ) )
                # Enpassant(front_tile, color, )
        ...
        for front_tiles in front_left_and_right_tile:
            ...
            if board.pawn_is_occupied_enemy(front_tiles, color):
                ...
                if front_tiles[1] == color.pawn_promotion_rank:
                    for promotion_option in promotion_list:
                        # add each promotion option to available moves
                        ret_list.append(Move(tile, front_tiles, promotion_option(color), None))
                    ...
                    # we just append
                    ret_list.append(Move(tile, front_tiles, Pawn(color), None))
        return ret_list
